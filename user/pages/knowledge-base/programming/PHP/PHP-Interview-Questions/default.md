...
Version 7.3 is the current version of PHP (as of *Sept 2019*).
...
To be able to display the output directly to the browser, we have to use 
the special *short tags* **\<?=** and **?>**.  
...
We use the *comparison* operator **'\=='** to test if two objects are instanced from the same class and have same attributes and equal values. We can test if two objects are referring to the same instance of the same class by the use of the *identity* operator **'==='**.
...
15. **What type of operation is needed when passing values through a form or a URL?**
If we would like to pass values through a form or a URL, then we need to encode and to decode them using *htmlspecialchars()* and *urlencode()*.
...
17. **What is needed to be able to use image functions?**
...
*file_get_contents()* is used for reading a file (or URL file) and storing it in a string variable.
...
30) **How is the result set of MySql handled in PHP?**
...
Imagine you have a form field called *'var'* on the form.  When the user clicks submit to the post form, you can then access the value of *'var'* like this:
```
$_POST["var"];
```
...
The function *get_magic_quotes_gpc()* tells us whether the *magic quotes* is switched on or not.
...
*"13"* and *12* can be compared in PHP since PHP casts everything to the integer type.
...
*$$var2* contains the value *10*.  This feature allows you to change the name of a variable dynamically.

59. **What does accessing a class via ***::*** means?**
***::*** is used to access static methods that do not require object initialization.

60) **In PHP, are objects passed by value or by reference?**
In PHP5+, objects are passed by *reference*.
...
65. **How do I initiate a session in PHP?**
...
74) **How can we get the error when there is a problem uploading a file?**
...
80) **What are the two main concatenation operators?**
...
82) **What is the difference between \$a != \$b and \$a !== \$b?**
...
101. **What is ***memcache***?**
A technology that caches objects in memory so your web app can get to them very fast.  This is an essential ingredient in *scaling*.

102) **Explain whether it is possible to share a single instance of a Memcache between multiple PHP projects?**
...
103. **Explain how you can update Memcached when you make changes to PHP?**
...
104) **What is ***cURL***?**
*cURL* is a library that lets you make HTTP requests in PHP.  In order to use, you must install the *libcurl* package. You can make HTTP requests without a URL, though it requires *allow_url_fopen* to be enabled in your *php.ini* file - you can do this with *file_get_contents()*.

105. **What are ***magic methods***?**
Magic methods are member functions that are available to all:
- Magic methods always start with “__” (i.e. 2 underscore chars)
- All magic methods need to be declared public
-  To use, it should be defined within the class or program scope.
**Examples**: 
     - __construct() - the constructor of a class; execute this code when class is instantiated.
    - __destruct() - the destructor of a class; execute this code when class object is destroyed.
    - __set() - Called when setting a member variable of a class; used to set the private property of the object.
    - __get() - Called when getting a member variable of a class; allows you to access a private property of an external object 
    - __call($funName, $arguments) - will be called when an undefined/inaccessible method is called.
    - __callStatic($funName, $arguments) - will be called when an undefined/inaccessible static method is called.
    - __toString() - called when using echo method to print an object directly
    - __sleep - called first when executing serialize()
    - __wakeup() - called first when deserialization() is executed
    - __isset() - called when calling isset() or empty() for an undefined or inaccessible (I.e. private) member.
    - __unset() - called when calling reset() for an undefined or inaccessible member.
    - __autoload() - called when you try to load an undefined class
    - __clone() - called when the object is copied
    - __set_state() - called when calling var_export().
    - __debuginfo() - prints debug information when the var_dump() method is executed.

106) **What is an ***interface***?**
...
*Abstract classes* are the classes in which at least one method is abstract. Abstract classes in PHP are declared with the help of the *abstract* keyword. Use of abstract classes are that all base classes implementing this class should give implementation of abstract methods declared in parent class. An abstract class can contain abstract as well as non abstract methods.